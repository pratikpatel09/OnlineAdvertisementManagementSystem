...
<?php
    ...
    if (isset($_POST['btnlogin'])) {
        if ($_POST['useremailid'] == "") {
            $logerr = "Please enter email id!";
            $flag = false;
        }
        if ($_POST['userpassword'] == "") {
            $logerr1 = "Please enter password!";
            $flag = false;
        }
        if ($flag == true) {
            $logid = $_POST['useremailid'];
            $logpwd = base64_encode($_POST['userpassword']);

            $query = "select u_firstname,u_email,u_password,u_type from user_master where u_email='" . $logid . "' and u_password='" . $logpwd . "'";
            $result = $mysqli->query($query);
            //$row=$result->fetch_assoc();
            if ($result->num_rows == 1) {
                $row = $result->fetch_assoc();

                $_SESSION['ses_logid'] = $row['u_email'];
                $_SESSION['ses_logpwd'] = $row['u_password'];
                $_SESSION['ses_utype'] = $row['u_type'];
                $_SESSION['ses_ufnm'] = $row['u_firstname'];
                //echo $_SESSION['ses_logid'];
                
                if($_SESSION['ses_utype'] == "publisher")
                {
                    header('location: publisher/pub_index.php');
                }
                else if($_SESSION['ses_utype'] == "advertiser")
                {
                    header('location: advertiser/adv_index.php');
                }
                else
                {
                ...
                $auth_error="Invalid Login id and Password";
            }
        }

        if (isset($_POST['remember'])) {
            $rlogid = $_POST['useremailid'];
            $rlogpwd = $_POST['userpassword'];
            if ($rlogid == $_SESSION['ses_logid'] && $rlogpwd == $_SESSION['ses_logpwd']) {
                setcookie("logidcookie", $logid, time() + 60*60*24);
                setcookie("logpwdcookie", $logpwd, time() + 60*60*24);
            }
        }
    }
    ?>
<html>
    <head>
        ...
        <style>
            ...
            .back-home:hover{color: #2962FF; text-decoration: none;}
            .back-home:link{color: #448AFF; text-decoration: none;}
            .back-home:visited{color: #448AFF; text-decoration: none;}
            .back-home:active{color: #2962FF; text-decoration: none;}
            .form-signin
            {
                max-width: 330px;
                padding: 15px;
            ...
            .reg-btn-color{color: #448AFF;}
            .reg-btn-color:hover{color: #2962FF;}
            .reg-btn-color:active{color: #2962FF;}
            .reg-btn-color:visited{color: #448AFF;}
            .reg-btn-color:link{color: #448AFF;}
            .reg-btn-color{font-size: 20px;}
            .reg-btn-color1{font-size: 20px;}
        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-6 col-md-4 col-md-offset-4">
                    <h1 class="text-center login-title">Log in to continue to <b><a href="index.php"><span class="span-logo-color">A</span>d<span class="span-logo-color">W</span>ale</b></a></h1>
                    <div class="account-wall">
                        <img class="profile-img" src="icons/profile_pic.png" alt="">
                        <div class="text-center">
                            <span class="text-danger"><?php echo $auth_error; ?></span>
                        </div>
                        <form class="form-signin" method="post"> 
                            <input type="email" class="form-control" name="useremailid" placeholder="Email" value="<?php if (isset($_COOKIE['logidcookie'])) {
        echo $_COOKIE['logidcookie'];
    } ?>" autofocus><span class="mylogerror"><?php echo $logerr; ?></span>
                            <input type="password" class="form-control" name="userpassword" placeholder="Password" value="<?php if (isset($_COOKIE['logpwdcookie'])) {
        echo $_COOKIE['logpwdcookie'];
    } ?>"><span class="mylogerror"><?php echo $logerr1; ?></span>
                            <button class="btn btn-lg btn-primary btn-block" type="submit" name="btnlogin">Log in</button>
                            <label class="checkbox pull-left">
                                <input type="checkbox" value="remember-me" name="remember">
                                Remember me
                            </label>
                            <label class="checkbox pull-right top" style="margin-top:10px;"><a href="passrecovery.php" class="text-info">Forgot you Password?</a></label>
                            <!--<a href="#" class="pull-right need-help">Need help? </a><span class="clearfix"></span>-->
                        </form>
                    </div>


                    <div class="text-center new-account bottom-block reg-btn-color1">
                        <p class="text-center new-account">Create an account</p>
                        <a href="Registration.php?usertype=advertiser" class="reg-btn-color">Advertiser</a> | <a href="Registration.php?usertype=publisher" class="reg-btn-color">Publisher</a>
                    </div>
                </div>
            </div>
        </div>
